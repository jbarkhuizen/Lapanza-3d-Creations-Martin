...
| status | TEXT NOT NULL DEFAULT 'Backlog' | `Backlog` / `In Progress` / `Done` / `Won't Fix` / `Claude Fix` |
...
| **Payfast live payment verification — fix shipped, unproven against a real transaction** | `PAYFAST_MODE=live` confirmed 2026-08-27 (#6 closed, stale). The passphrase was then confirmed by the owner to match the Payfast dashboard exactly, which pointed at a real code bug instead: `buildSignature()` was dropping blank-valued ITN fields (`custom_str1` etc.) before hashing, contradicting Payfast's own reference algorithm, which includes them. Fixed in `server/payfast.js`, covered by a new regression test — see §2 Evolution History. Still can't be marked fully verified until a real payment actually completes end-to-end and lands as `paid` (backlog #45 stays open for that). |
...
| **Overnight batch's admin-only features still not click-tested against a real admin session** | Deployed live 2026-08-28 (V0.58/V0.59) after the owner reviewed and approved. Public-facing pages were verified live post-deploy (see row 121, §2). Per-item catalog save/remove, the New Order client-detail panel, invoice emails, and the "Updating Client Data" toast are still only verified by code review + the full automated test suite (299/299) — no admin session was available to click through them from this environment. Worth a real pass (tick a GWM item's models and Save item, place a real test checkout, confirm the invoice email actually arrives) next time there's admin access in a session. |
