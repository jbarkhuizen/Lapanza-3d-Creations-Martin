...
| **Payfast live payment verification — open** | Confirmed 2026-08-27: `PAYFAST_MODE=live` in `.env` (the mode flip itself is real and done — backlog #6 closed as Won't Fix, stale). What's still unverified is whether a live payment actually completes end-to-end: the one real transaction on record (Aug 26, order `691b035e`) failed ITN signature verification — Payfast confirmed it as paid, this server didn't, so the order never auto-transitioned to `paid`. See the correction row in §2 Evolution History. Needs the business owner to check `PAYFAST_PASSPHRASE`/`PAYFAST_SANDBOX_PASSPHRASE` in `.env` against the Payfast merchant dashboard before this can be called verified (backlog #45). |
