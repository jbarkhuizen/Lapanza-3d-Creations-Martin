...
| `node --test` | Node's built-in test runner — no Jest/Mocha/Vitest. 232 tests across 27 `*.test.js` files (current count — see §14 for the authoritative figure) |
...
- `category` is one of `Bug`/`Feature`/`Enhancement`/`Tech Debt`; `status` is one of `Backlog`/`In Progress`/`Done`/`"Won't Fix"`/`"Claude Fix"` — an invalid value on create silently falls back to `Feature`/`Backlog` rather than rejecting the request (mirrors `createPurchase`'s status-clamping pattern elsewhere in this codebase).
...
- **Current count:** 232 tests across 27 test files, 100% passing at last recorded run.
