...
| **Buying price and the Stock Value sheet (2026-09-07, owner request)** | Stock Management now edits a Buying Price per item alongside the renamed Selling Price (both sortable): filament colours store it in a new `filament_colours.buying_price_rand` REAL column (addColour/updateColour round-trip it), category items carry `item.buyingPrice` inside catalog.json (normalizeItem preserves it through catalog edits, `updateCategoryItemStock` writes it, cents allowed). It is deliberately absent from every public export -- syncPublicJson's explicit field lists for filaments and categories omit it. A new **Stock Value** page under the Expenses nav group shows the true value of stock on hand: per in-stock item, quantity x buying price, with stat cards for total value at cost, potential sale value, potential margin, and a count of items still missing a buying price (flagged red in the table so the total stays honest). Client-side compute over the existing /api/inventory list. 1 new test. | 462/462 unit |
| **Orders list: Client column and Paid/Packed ticks (2026-09-07, owner request)** | The Orders table now shows the client's name (from the client record) and two additional checkboxes in workflow order ahead of the existing three: Paid, Packed, Collected, Shipped, Finalized. Paid is a view onto the real status machine -- ticking marks the order paid (untick returns it to pending payment); Payfast-paid orders arrive already ticked, so the box exists for manual-EFT and cash orders. Packed records an independent `orders.packed_at` timestamp (same pattern as Collected: `setOrderPacked`, audited `PATCH /api/orders/:id/packed`) and never changes order status, so it survives payment or shipping transitions. All boxes stay disabled on cancelled orders. 1 new test. | 461/461 unit |
| **Expenses module + Financial Overview (2026-09-07, owner request)** | A new EXPENSES nav group with two pages. **Expenses**: capture supplier invoices with multiple line items (description, category, quantity, unit price -- live line and invoice totals), Service Provider / Date Purchased / Paid Via / Notes on the header; searchable, sortable list with edit and delete; amounts stored as REAL rand with cents (supplier invoices carry cents, unlike the integer-rand catalog convention). Categories and payment methods are new configurable Settings lists (`expenseCategories` seeded with Printers & Equipment / Filament & Stock / Consumables / Operating Costs; `expensePaymentMethods` seeded with the owner's five cards/accounts). **Financial Overview**: stat cards (this month in/out/net + current tax-year net), a 12-month income-vs-expenses chart, category breakdown, per-card totals, SA tax-year summaries (1 Mar - end Feb, current + previous), and a month-by-month table. Income counts paid orders only (payment_status = 'paid', cancelled excluded) -- owner decision. **Purchase History is retired**: its flat rows migrate on every boot into one-line expense invoices (idempotent via `source_purchase_id`; the purchases table itself is kept untouched as a safety copy) and its page and API routes are removed. Backend `server/expenses.js`; routes: GET/POST `/api/expenses`, GET/PUT/DELETE `/api/expenses/:id`, GET `/api/finance/overview` (admin auth, writes audit-logged). 6 new tests. | 460/460 unit |
| **Google address autocomplete at checkout (2026-09-06, owner request)** | A Search your address box now sits above checkout's address fields: powered by Google Places (PlaceAutocompleteElement web component, South Africa only), one selection parses the address components into street/suburb/city/province/postal code, all of which remain editable. The Maps key lives in the new `googleMapsApiKey` setting -- deliberately included in the public site-settings export, because a Maps JS key is embedded in the page by nature; its protection is the domain + API restriction configured in the Google Cloud console, and the admin Settings label says so. Loaded only when a key is set; every failure path (no key, blocked script, API error, component API drift -- both the current gmp-select and legacy gmp-placeselect events are handled) silently leaves manual entry as it was. | 455/455 unit |
| **PUDO locker selection + customer notes at checkout (2026-09-06, owner request)** | Checkout's PUDO Locker method now captures WHICH locker the parcel goes to. `server/pudo.js` proxies The Courier Guy's locker list (api-pudo.co.za/api/v1/lockers-data; requires the free API key from a pudo.co.za account, entered in Settings as `pudoApiKey` -- server-side only, never in the public settings export) with a 24h disk cache (`data/pudo-lockers.json`), a stale-cache fallback when the courier API is down, and a public slim endpoint `GET /api/pudo/lockers`. On checkout: a type-ahead locker search (name/suburb/address) with a chosen-locker card and Change button; when no locker list is available the picker degrades to manual name+address fields, so PUDO checkout never blocks. Picking PUDO also stops requiring the street-address fields (the locker is the address; Local Delivery still requires them), and submit refuses PUDO orders without a locker. A new optional Notes / Special Instructions textarea rides along on every order. Both stored on the order (`pudo_locker_name`, `pudo_locker_address`, `customer_notes` -- trimmed and capped at 200/400/1000 chars) and surfaced on the admin order detail (Delivery Notes panel), the owner notification email, and the packing slip. Checkout panels now numbered 1-4. 4 new tests. | 455/455 unit |
| **Instruction files attached to order emails (2026-09-06, owner request)** | The order detail page gained an Instruction Files panel: a checkbox per uploaded instruction sheet plus a Save selection button. The selection is stored on the order itself (`orders.instruction_files`, a JSON filename array; `setOrderInstructionFiles`; `PATCH /api/orders/:id/instruction-files`, which rejects filenames not in the library and audit-logs every change). The customer's confirmation email and invoice email then carry the ticked sheets as PDF attachments (`instructionAttachments` in mailer.js -- basename-guarded, and a sheet deleted from the library after being picked is skipped silently rather than blocking the email). Workflow: open the order, tick the sheets, Save selection, then Send/Resend the confirmation or invoice email. 1 new test. | 451/451 unit |
| **Instruction Files (2026-09-06, owner request)** | A new admin page (Local Management -> Instruction Files) for printable instruction sheets -- fitting cards and similar -- that the owner includes with shipped prints. Upload (PDF only, max 10MB), list (name/size/date), View / Print (opens the public /uploads/instructions/<name> URL in a new tab), Delete (confirm + audit-logged). Implementation: `server/instruction-files.js` -- multer disk storage into public/uploads/instructions (same untracked-uploads deployment story as product images, included in the rclone offsite sync), filenames sanitized from the original name so they stay recognizable, extension always .pdf from the validated mimetype, name collisions get a hash suffix instead of overwriting, deletes are basename-guarded against path traversal. Routes: GET/POST/DELETE `/api/instruction-files` (admin auth, upload/delete audit-logged). 3 new tests. | 450/450 unit |
| **Sortable columns on Stock management and Reorder Report (2026-09-04, owner request)** | Both tables gained clickable column headers (client-side sort, first click ascending, second descending, arrow shows active column/direction; shared `applySort`/`sortableTh` helpers). Stock management sorts by SKU, Name, Stock, Price, Remaining (filament) and Products page (Listed before Not listed) -- applied within each category section, with unsaved cell edits surviving the re-render; rows missing a value (e.g. Remaining on non-filament items) always sort last. Reorder Report sorts by Item, SKU, Category, In Stock and Sold (30 Days), re-rendering from the already-fetched list without hitting the API again. | 447/447 unit |
| **Reorder Report on its own page (2026-09-04, owner request)** | The reorder report (#122) moved out of the Stock management page (where it sat as a collapsible panel above the stock table) into its own page: new `Reorder Report` nav entry under Local Management, route `reorder-report`, rendered by `renderReorderReport()` in admin.js. Backend unchanged (`GET /api/reorder-report`); the page shows the same table -- item, SKU, category, in-stock (red at 0), sold last 30 days -- against the Settings -> Storefront low-stock threshold, with a clean empty state. Stock management now contains only stock editing. | 447/447 unit |
| **Optional client email for walk-in customers (2026-09-03, owner request)** | Walk-in customers captured directly in the admin centre often have no email, so it is no longer mandatory on admin paths: the New Order new-client fields and the Clients page form both accept an empty email (labels now say Optional), `createClient`/`updateClient` no longer throw on empty, and `createManualOrder` passes `requireEmail: false` to `findOrCreateClientForCheckout`. Online checkout and account registration still require email (their invoices/confirmations depend on it). Safety: `findClientByEmail` now returns null for an empty lookup, so two different no-email walk-ins can never silently merge into one client record (an identical first+last name still reuses the record, the existing exact-name fallback). The manual-order route skips the customer invoice email cleanly when the client has no address instead of logging a delivery failure. 1 new test. | 447/447 unit |
| **Client editing from order detail + New Order fixes (2026-09-03, owner request)** | (1) The order-detail Client panel now has an Edit button opening an inline form (first name, surname, business, email, phone, street, suburb, city, province, postal code); saving updates the client record itself (PUT /api/clients/:id, partial merge), so the correction appears on every order and list that client is part of. (2) New Order bug fixed: typing new-client details then touching any other part of the form (adding a line, picking a product, changing shipping) wiped them, because those interactions re-render the whole view from state and the client inputs never wrote to state -- they now persist on every keystroke. (3) Underlying `updateClient` bug fixed: updating firstName/lastName without an explicit `name` left the composed display name stale (how INV-0024 kept showing the wrong name); it now recomposes from first + last, explicit `name` still wins. 1 new test. | 446/446 unit |
| **Automated order cancellation removed (2026-09-03, owner request)** | The hourly auto-cancel job (`startAutoCancelJob` in jobs.js, `cancelStalePendingOrders` in orders.js) that cancelled `pending_payment` orders unpaid after 7 days has been deleted per owner instruction. Cancellation is now exclusively a human decision: the admin status dropdown (`PUT /api/orders/:id/status`) or the customer's own account-page Cancel button (`POST /api/client/orders/:id/cancel`) -- both unchanged, both still release reserved stock and notify the owner. Operational consequence: abandoned checkouts keep their stock reserved indefinitely until someone cancels them, so stale pendings are now the owner's manual housekeeping. | 445/445 unit |
| **Orders fulfilment tracking (2026-09-03, owner request)** | The admin Orders list now shows the invoice number and three per-order tick boxes: Collected, Shipped, Finalized. Collected records the physical handover in a new `orders.collected_at` column (independent of status -- courier orders can also be collected; `PATCH /api/orders/:id/collected`, audit-logged). Shipped and Finalized are views onto the existing status machine: ticking Shipped sets status `shipped` (untick returns to `paid`), ticking Finalized sets `completed` (untick returns to `paid` for collected orders, `shipped` for courier ones). Collected and Shipped are mutually exclusive -- an order is either handed over in person or sent by courier, so ticking one clears the other; Finalized (delivery/handover confirmed) works from either fork. The list keeps both the order-id and invoice-number columns -- both through the audited `PUT /api/orders/:id/status` route, so cancellation restock and payment-status lockstep still apply. Boxes are disabled on cancelled orders. Fixed in the same change: the manual-order route referenced the checkout-only `reusedOrder` variable, a ReferenceError that had silently suppressed the owner-notification email for admin-created orders since the duplicate-guard commit. 1 new test. | 446/446 unit |
| **New-order notification upgrade (2026-09-03, `ee9b537`, owner request)** | The owner email (settings.orderNotificationEmail) now carries the complete order in its body — item table, totals block (volume + promo lines), payment method, client contact and delivery address — and attaches `packing-slip-<INV>.html`, a print-ready document for the shipping box (open the attachment, click Print). The packing-slip template moved to `server/packing-slip.js` (shared by the admin `GET /api/orders/:id/packing-slip` route and the mailer; avoids a mailer→index circular import) and gained the letterhead logo + invoice number. 1 new test. | 445/445 unit + 4/4 e2e |
| **Duplicate-order guard + payment retry (2026-09-03, `4d889ce`)** | Go-live incident: Payfast's cancel_url returns to checkout with the cart intact (deliberate), and each re-submission created a new order — three real customers produced 4 duplicate pendings alongside their genuine orders (nobody double-charged; paid: INV-0018 R980, INV-0021 R1,748). Fixes: (1) `createOrder` reuses an identical pending order from the same email (same item multiset + total, ≤30min old) as a payment retry — payment-method switches update the existing order, the `_reused` flag suppresses re-sending invoice/confirmation/owner emails, and changed carts or non-pending priors still create fresh orders; (2) `POST /api/checkout/retry-payment` (checkout-limited, pending Payfast orders only, order-UUID capability) re-enters Payfast on the same order, with a card/EFT choice; (3) checkout shows a retry banner when arriving via `?cancelled=<orderId>`. Duplicates cancelled with stock released. Verified on a scratch stack end-to-end. 2 new test blocks. | 444/444 unit + 4/4 e2e |
| **Communications layout rework (2026-09-02, `72bc811`, owner request)** | Settings → Communications rebuilt catalog-style: 13 templates in two collapsible groups (Customer Emails ×9, Notifications to You ×4), one collapsible card per template — the summary shows the template name plus its CURRENT subject line; inside are a when-it-sends description, subject, a taller message box, and clickable token chips that insert the placeholder at the message cursor (`EMAIL_TEMPLATE_META` gained `group`/`when` and `tokens` became an array). Save wiring unchanged — the same `data-email-template` inputs feed Save Communications; closed cards still save since their inputs stay in the DOM. Browser-verified on a scratch stack. | 443/443 unit + 4/4 e2e |
| **Evening batch: New Order UX + analytics upgrades (2026-09-02, `83972d2` / `af5f46a` / `63a17db`)** | **New Order pickers** (owner feedback): client + product selection are live type-ahead — client search debounces 250ms against /api/clients, product search filters the cached inventory instantly; results inject into their own container only (a full re-render steals input focus), and the whole result row is the select button. The old Enter-to-search + separate "Use"-click flow is gone. **Analytics hourly chart**: `getVisitSummary().hourlyTraffic` — visits + unique visitors per hour over the trailing 24h, 24 zero-filled buckets in server localtime, rendered as a grouped-bar inline SVG with legend/labels/hover. **Analytics ranges**: Shopping Funnel + Top Pages get range dropdowns (1h/24h/7d/30d; Top Pages also All Time via the permanent tallies). `getEventSummary` signature changed to `(range, db)`; `getTopPages(range, db)` added; ranges are a fixed `ANALYTICS_RANGES` vocabulary — unknown values fall back, nothing user-supplied reaches SQL. 3 new tests. | 443/443 unit + 4/4 e2e |
| **Deploy incident + guard, and dynamic category pages (2026-09-02, `099dc8d` + `2eaab7b`)** | **Incident**: a deploy that skipped the sync-from-DB/generate runbook steps baked git's committed generated files (DEV-database catalog) into dist/ — live category pages briefly showed the dev catalog, missing products and photos. Restored by resync+regenerate+rebuild (verified against catalog.json counts). **Permanent guard: deploy-app.sh itself now runs syncPublicJson(live DB) + `npm run generate` before every build** — no deploy path can skip them (only a first bootstrap with no DB bypasses). **Dynamic category pages** (owner-requested after the Weed Shop category shipped page-less): every PUBLISHED category in categories.json that isn't core (toys/homeware/phones) or a car-part brand generates its own root page with item detail pages, search-index and sitemap entries; drafts stay admin-only; the public categories export now carries `status`; CORE_CATEGORY_NAV appends dynamic categories after the core three. **vite htmlEntries scans the repo root for *.html** and adds anything not hand-listed — the recurring "new root page 404s in production" trap is structurally gone. Admin flow for a new category: +Category → items → Published → Publish to site. Live-verified (/weed-shoppe.html + sidebar link). 1 new test. | 441/441 unit + 4/4 e2e |
| **Afternoon follow-ups (2026-09-02, `25e1215` + `a64dc43`)** | #7 WhatsApp Deferred per owner — the backlog reached zero active items. TikTok profile link restored on get-in-touch.html (owner-confirmed @jbarkhuizen). Social launch kit committed under docs/marketing/ (three branded graphics + per-platform posting playbook). Promo fix: `updatePromoCode` treated `expiresAt: null` as "keep existing" (`??` fallback), so a promo's expiry could never be cleared — the admin date field sends null when blanked. Semantics now: undefined = keep, null/'' = explicit clear (regression test). Prod promo state after the owner's own admin edits: LAUNCH5 (5% off, min R250, no expiry) is the single launch code — it IS the renamed Sprint10 record; the briefly-created LUNCH5 typo duplicate is deactivated with 0 uses. | 440/440 unit + 4/4 e2e |
| **Owner UI review — 27/27 shipped in one day (2026-09-02, batches A `4751917` / B `f00c085` / C `2213faf` / D `c106dc7` / logo `d5a103b`; review items logged as todos #140–#166, review #N = todo #139+N)** | **A (quick UI)**: doc last-updated stamps; revenue-chart legend + endpoints; dark-mode select fix (explicit option colors — Windows Chrome paints a light popup while options inherit dark-mode light text); Budget R prefix; Review&Send hides while the review panel is open; uniform hero CTAs (owner reversed #36's hierarchy); password eye toggle in both bundles. **B (bugs)**: storage stats via `du -sx` (stat() on a dir = constant inode size); missing category status normalizes to published on read (list said Draft, editor showed Published); Top Products excludes `manual:` lines; **sharp variants `.rotate()` first — WebP drops EXIF, whole library regenerated (375 variants)** + one image hand-rotated with backup; brands without a matching category filtered from nav/generator/sitemap instead of 404ing; DR detail GET/PATCH/quote responses attach `files` like the list + thumbnail/download files block. **C (data/ops)**: offsite-uploads copy failures audit-logged (were console-only — offsite had silently fallen to 21MB vs 90MB local; caught up to byte-identical; Drive throttles rclone's shared client_id ~220KiB/s, which retires during 2026 — owner to create an own client_id); in-house filament Archive (history kept, picker-hidden, reversible); catalog+stock groups from real category names (`dynamicGroupDefs`); 22 clients imported to Potential Market "Initial Load"; publish history — `catalog_published` audit events + `/api/publish-history` + dashboard last-10 panel. **D (features)**: `CORE_CATEGORY_NAV` dynamic sidebar names ("Home & School" live); 'quoted' as a first-class DR status at every layer (auto-set on quote send); `settings.quoteTermsDefault` prefill ({{depositPct}} substituted); per-item product videos (`uploadCategoryItemVideo`, MP4/WebM ≤50MB, `videoUrl` carried through both save paths + export + detail-page `<video>`); `uploadFormData()` XHR progress overlay on all admin uploads. **Logo (#162)**: `public/branding/` asset pack (transparent logo, favicons, apple-touch, 1200×630 og-image) wired into head()/hand-written favicons (404.html absolute paths), default og:image, sidebar brand, admin, email cream-band header, invoice letterhead, `/branding` Express mount — closes the launch audit's og:image leftover. | 439/439 unit + 4/4 e2e |
| **#99 promo codes (2026-09-02, `e28a83e`; gift-card half deliberately parked — prepaid liabilities need balance accounting)** | `promo_codes` table (code UNIQUE COLLATE NOCASE; kind percent/fixed; value; min_subtotal; expires_at; max_uses NULL=unlimited; used_count; active) + `orders.promo_code`/`promo_discount_amount` (`ensurePromoColumns`) — SEPARATE from discount_pct/discount_amount so volume + promo both show honestly. `server/promos.js`: CRUD (`normalizePromoInput` caps percent at 90, validates code charset), `validatePromo(code, subtotalAfterVolume)` → ok/customer-safe reason, `computePromoDiscount` (rounded, never exceeds base), `redeemPromo` — guarded UPDATE (`used_count < max_uses`) that MUST run inside createOrder's transaction: concurrent checkouts can't over-redeem, and a thrown redemption rolls the whole order back. **Stacking (owner-approved): volume discount first, promo on the remainder.** An invalid code at order creation THROWS (customer typed it expecting a discount — silently charging full price would be worse). Routes: public `POST /api/promo/validate` (checkoutLimiter — unauthenticated guessing surface; preview only), admin `GET/POST/PUT /api/promo-codes` (audit-logged as settings_updated). Checkout page: promo input + Apply → validate → discount row + live total; code rides the payload, server stays authoritative. Admin: Promo Codes page (route `promos`, shipping-options pattern; expiry stored end-of-day so "expires the 15th" works ON the 15th). Invoice gains a promo row; the order-email totals table now renders BOTH discount rows (it previously showed none). Verified end-to-end in a browser: apply/reject/re-apply, order submitted, DB row + use count confirmed. 7 new tests. | 436/436 unit + 4/4 e2e |
| **#139 rich-text product descriptions (2026-09-02, `bbc9e32`)** | Owner-scoped and approved (full toolbar, all description/text fields, links allowed). `server/rich-text.js`: `sanitizeRichText()` reconstructs limited HTML from an allowlist (p/br/strong/em/u/s/h3/h4/ul/ol/li/a/blockquote — aliases b→strong, i→em, div→p; only http(s) hrefs survive, gaining rel="noopener noreferrer" target="_blank"; text is entity-decoded then re-escaped so the function is idempotent; unbalanced tags are closed, orphan closes dropped) and `stripTags()` projects to plain text. Enforced on save in `createFilament`/`updateFilament` (description, colour_note), `POST/PUT /api/products` (description) and `normalizeItem()` (details) — and applied again at render time in `generate-pages.mjs`, which makes pre-#139 legacy values safe: those fields were previously interpolated into public HTML with no escaping at all. Meta descriptions, the colour-detail meta line and the category-card `data-search` index go through `stripTags()`. Render containers switched from `<p>` to `<div class="rich-text">` (valid block nesting) with styles in src/styles/main.css (Tailwind preflight strips list/heading rendering). Admin (`richTextField()` + delegated handlers in admin.js): toolbar (B/I/U/S, H3/H4/¶, bullet + numbered lists, Link/Unlink https-only via prompt, Clear) over a contenteditable div; the field's original hidden `<textarea data-field/data-item>` is kept and synced on input so every existing binding and save reader is untouched; a MutationObserver initializes editors after any panel render, filling them through a DOM-based client mirror of the allowlist. Chrome findings encoded in the implementation: a contenteditable inside `<label>` silently cancels editing commands; `execCommand('bold')` split text nodes without wrapping (inline formats toggle manually via `rtToggleInline`); `styleWithCSS` must get boolean false, not the string. Verified in a real browser against a scratch-DB server (editor render, every toolbar command, textarea sync, byte-stable server round-trip) and live after deploy. 8 new tests. | 429/429 unit + 4/4 e2e |
...
| **Flash Stock Specials — time-limited, quantity-capped price drops on filament colours** | Owner request: run a special on a defined stock quantity for a set number of days (worked example throughout design: "supplier has 10 rolls of Black PLA at a discount for 2 days"), tracked separately from normal stock, auto-reverting once that stock sells out, endable by hand at any time. Presented two architectures first as a standalone design doc (comparing "layer 4 fields on the existing colour row" against "give the special its own row/SKU/stock pool") before writing any code; owner picked the latter, filament colours only for now. **The key design win, found while implementing**: a special doesn't need a NEW productId scheme the way the original proposal assumed (which would have touched checkout/cart/orders/invoices everywhere a productId is parsed) — it's simply a second `filament_colours` row, reusing the exact same `filament:{slug}:{sku}` id every other colour already uses. `resolveProductSnapshot()` and every stock function in `orders.js` (`reserveStockForOrder`/`decrementStockForOrder`/`restoreStockForOrder`) already handle any `filament_colours` row generically by SKU — a special is invisible to all of them as a concept, so **checkout needed zero new code**. 5 new columns (`server/db.js`'s `ensureSpecialsColumns`): `special_status` (NULL/'active'/'ended', kept not deleted on end — the only "how did that batch do" history this feature has), `special_source_colour_id`, `special_started_at`/`special_ends_at`, `special_was_price_rand` (a snapshot for the storefront's "was RXXX" display, deliberately never re-read from the base colour so it can't drift if the base price later changes), `special_initial_qty` (fixed at start, the only source for "N of 10 left" once `stock_qty` — which decrements on every sale like any other colour's — has moved). **`startSpecial()`/`endSpecial()`** (`server/filaments.js`): starting decrements the base colour's `stock_qty` by the allocated quantity and inserts a new row named `"{base} (Special)"` (disambiguates it from the base on any surface — cart, invoice, admin lists — that shows plain names side by side; the storefront's own badge does the rest) with a fresh unique SKU (`{baseSku}-SPECIAL`, falling back to a random suffix if that colour's had a special before); ending — by any of the three routes below — merges whatever's still unsold back onto the base colour's `stock_qty` (0 in the sold-out case, a real number in the early-end/expiry case) before marking the row `'ended'` and unlisting it, which is the literal mechanic behind "if sold go back to standard pricing": units SOLD during the special are just sold, units NOT sold rejoin the standard-price shelf. **Three ways a special ends**, all converging on `endSpecial()`: an admin's own "End Special Now" (immediate, via the new Specials admin page); `special_stock_qty` reaching 0 through an ordinary sale (needs no special-aware code in the checkout path at all — see above; caught within 15 minutes by the sweep below, not synchronously, since an already-zero-stock row is already correctly unbuyable either way); and the sweep job noticing `special_ends_at` has passed. **`startSpecialsSweepJob`** (`server/jobs.js`, same in-process-`setInterval` pattern every other scheduled job in this file uses, every 15 min) calls a new `endExpiredAndSoldOutSpecials()` and republishes once if anything actually ended — `publish` is passed in as a parameter rather than imported, so `jobs.js` never needs to import from `server/index.js` (which already imports `jobs.js`), avoiding a circular import. **Admin**: new "Specials" page (Client Side nav, next to Promo Codes — the closest existing precedent for this page's shape: a create-form + status table, `renderSpecials()` mirrors `renderPromos()` almost exactly) — pick a colour from `GET /api/specials/candidates` (real, non-special, in-stock colours only), set special price/quantity/days/optional per-batch buying price, see every past and active special with its own "X of Y left" and time-remaining. Real gap found and closed while building this: the *generic* colour editor's existing "Remove" button (`deleteColour`) had no idea what a special was and would have hard-deleted an active one, silently skipping the merge-back and losing real stock — `deleteColour()` now refuses to delete a row with `special_status = 'active'` ("end it from the Specials page first"); ended specials (ordinary history at that point) delete normally. **Storefront**: `generate-pages.mjs`'s `colourCards()` adds a lime "Flash Sale" badge, a struck-through "was" price, a stock progress bar (from `specialInitialQty`), and a countdown element carrying `data-special-ends` — filled in and ticked down **client-side** (new `hydrateSpecialCountdowns()`, `src/js/site.js`) from the raw ISO timestamp rather than baked in as static text, so it never freezes at whatever it read at the last publish; the actual cutoff is still enforced server-side regardless (this is display only). `server/export.js`'s public JSON export exposes only the public-safe subset (`isSpecial`/`specialEndsAt`/`specialWasPrice`/`specialInitialQty` — never the buying price or the internal source-colour id), same explicit-field-list discipline already applied to `buyingPrice`. **Stock Value and Stock Management needed zero code changes** — since a special is a real `filament_colours` row with its own `buying_price_rand`, `listInventory()` (`server/inventory.js`) already iterates every colour under every filament type with no filtering, so a special (and, after it ends, its own zero-stock history row) shows up as its own correctly-priced line automatically. 11 new tests (`filaments.test.js`, all unit-level against the real functions and a real in-memory DB, matching this project's existing testing convention for admin-CRUD feature families like Promo Codes — no HTTP-level tests added for the same reason none exist for promo codes either): split-and-price-correctly, over-allocation rejected, one-active-special-per-colour enforced, special-of-a-special rejected, early-end-with-leftover merges correctly, sold-out-end needs no merge, unknown-id rejected, the sweep's dual expired/sold-out/still-healthy discrimination, `listSpecials`/`listSpecialCandidates` shape and filtering, SKU uniqueness across a second special after the first ended, and the delete-guard. 473/473. **Live-verified 2026-09-08** (owner supplied production admin credentials for this session specifically): created a real 1-unit test special on production PLA Black (R225→R199, 1 day), confirmed the admin table row (Active, "1 of 1 left", correct SKU suffix), confirmed the storefront `filament/pla.html` card rendered the Flash Sale badge, struck-through price, live countdown, and stock bar, then ended it via "End Special Now" and confirmed the row flipped to Ended and the base PLA Black colour's stock correctly restored to 15/15 — production left clean, no leftover test data. (One tooling note, not a product bug: this session's browser-automation tool doesn't surface native `window.confirm()` dialogs, so the End Special Now click needed `window.confirm` stubbed to `true` for that one verification click — the real confirm dialog a human admin sees is unaffected.) |
| **Manufacturing cost + made-to-order flag, split from buying price (2026-09-08, owner request)** | Immediate trigger: a bulk buying-price update (owner asked for every filament colour set to R180 as a placeholder — done via a scripted `/api/inventory` bulk call, 179/179 rows) surfaced that the Stock Value sheet's cost-of-stock-on-hand model didn't fit category (printed) items at all: **"UNo Box shows Stock 5, but we don't have a true 5 in stock — these are printed on demand."** A buying price implies something was bought; a printed item's real cost driver is manufacturing cost (materials + time, eventually sourced from a costing sheet the owner does per item) — and a nonzero `stockQty` on a made-to-order item doesn't mean physical units are sitting on a shelf, so counting it into "stock value" overstated the true figure. Two new category-item-only fields (`server/index.js`'s `normalizeItem()`; **filament rows never get either field** — a filament roll is always genuinely bought stock, so `listInventory()`'s filament branch hardcodes `manufacturingCost: null` / `madeToOrder: false`, the `null` specifically distinguishing "not applicable to this row" from "0 / not entered yet"): `manufacturingCost` (Rand, same `buyingPrice`-style clamp/round), and `madeToOrder` (boolean, **defaults `true`** — `item.madeToOrder !== false` — deliberately the opposite default polarity of every other boolean flag in this codebase, because today almost the entire catalog genuinely is printed on demand; defaulting to `false` would have kept the Stock Value totals just as wrong as they already were until every existing item was hand-flagged one at a time, and defaulting `true` is what actually makes the number honest immediately). `server/inventory.js`: `updateCategoryItemStock`/`bulkUpdateInventory` accept and validate both fields (negative manufacturing cost rejected, same pattern as buying price); both admin-only, never added to `export.js`'s explicit public-field allowlist (same discipline `buyingPrice`/`sourceUrl` already established — a field simply isn't there unless deliberately added to that list). **Stock Management** (`admin/admin.js`): two new grid columns — Manufacturing Cost (R), editable for category rows, reads "—" for filament rows (no input rendered, not just disabled); Made to Order / Stocked, a per-row radio pair, filament rows again read "—". **Stock Value**: an `effectiveCost(item)` helper (manufacturing cost if set, else buying price — covers category items that are actually bought components like hardware/inserts, not printed) drives every total; the three stat cards (cost value, sale value, margin) and the "missing a cost" flag now only sum `!madeToOrder` rows, with a new "Made-to-order items (excluded)" count card. Made-to-order rows still render in the table (own Manufacturing Cost column, dimmed row, a "Made to order" badge, "not counted" in the Stock Value column) so the owner can see per-item manufacturing cost without it polluting the "real stock on hand" figure — an item moves into the counted total the moment the owner flips it to Stocked on the Stock management page (the intended flow once the owner "start[s] to print some stock on hand items", per the request). Financial Overview (income vs. expenses) was **not** touched — it has no inventory-value concept today; the owner's "not true" complaint was about Stock Value specifically, confirmed by their own worked example. 3 new tests (2 in `inventory.test.js` covering the filament/category default split and the bulk-update round-trip + negative-cost rejection, 1 route-level in `index.test.js` covering `POST /api/products` normalization defaults and confirming both fields stay out of the public `categories.json` export). 476/476. **Live-verified 2026-09-08**: confirmed both new Stock Management columns render (Manufacturing Cost editable / "—" for filament, Made to Order↔Stocked toggle), confirmed the real save round-trip (`manufacturingCost` persisted via the Save Changes button, not just the API), and confirmed Stock Value's split on real production data — R11,880 at-cost value from 33 real filament rows only, 207 made-to-order category items correctly excluded, and the owner's own UNo Box example showing "not set"/"not counted" exactly as designed. |
| **Rand-prefixed money inputs + unsaved-changes nav guard (2026-09-08, owner request, `733c751`)** | Two owner reports from actually using the manufacturing-cost feature above. **Money formatting**: Buying Price / Manufacturing Cost / Selling Price inputs on Stock Management were bare `<input type="number">`s with no currency cue and no forced decimal display — owner wanted `x.xx` (e.g. `12.99`) with an "R" right in front of the field, not just a "(R)" column header. New `.rand-input` CSS wrapper (`admin.css`): a `::before` pseudo-element renders a fixed "R" inside the input's own padding, using the existing `--panel`/`--ink`/`--muted` theme tokens so it holds in dark mode too; `stockRowHtml()` (`admin.js`) wraps the three money cells in it and formats their value attribute with `.toFixed(2)` for the two cent-precision fields (Buying Price, Manufacturing Cost — Selling Price stays whole-Rand, matching this codebase's existing integer-rand catalog-price convention everywhere else). **Unsaved-changes guard + real browser history**: two related gaps the owner hit navigating around Stock Management with edits still on screen — leaving the page (sidebar click OR the browser Back button) silently discarded `state.stockEdits` with zero warning, and Back left the admin site entirely rather than moving to the previous admin page, because this SPA had never once touched `history` — every route switch was pure in-memory state with no URL/history entry behind it, so there was only ever the one entry the page loaded with. Fixed together: `setRoute()` now calls `history.pushState`/`replaceState` (first call replaces, every one after pushes) with `{route, id}`; a new `popstate` listener finds the matching `.nav-btn` for the popped route and replays a real click through it (`replayHistoryRoute()` — routes with no matching button, i.e. `order-detail`/`editor`/`version-detail`, fall back to their own list page via `POPSTATE_FALLBACK_ROUTE` instead, since there's no generic way to re-open "order X" from a bare route name); a `beforeunload` listener is the browser-level backstop for real navigation away (tab close, refresh, typing a URL — this never fires for the popstate path since Back/Forward within an SPA doesn't unload the document, so the two listeners are complementary, not redundant). The actual guard: `hasUnsavedChanges()` checks `state.stockEdits` (the only page in this admin that currently tracks a distinct saved-vs-draft state to compare against — the product catalog editor's `state.draft` writes straight through with no baseline snapshot, so it has nothing to detect yet and isn't wired in; a documented note in the code flags it as the obvious next candidate) and `guardNavigation()` shows a custom three-way modal (`showUnsavedChangesModal()` — Stay / Discard / Save; a native `confirm()` only gives two buttons and no custom labels) before both the nav-btn click handler and the popstate handler proceed; choosing Save calls a newly-extracted `saveStockEdits()` (previously inline in the `#save-stock` click handler only, now shared) and only proceeds with the navigation if it actually succeeded — a failed save leaves the admin on the page with the error toast still visible instead of navigating away from it. Verified live on production: the modal renders correctly on a real sidebar-click-away with a dirty Manufacturing Cost field, "Save changes" both persists the edit and completes the navigation, and the browser Back button now moves correctly through real admin page history (Dashboard → back → Stock management → back → Registered users) instead of leaving the site. No server-side changes; admin.js/admin.css only, no new tests (this admin has no unit-test coverage for its own JS, matching existing project convention — verified via live browser session instead). |
| **Unsaved-changes guard extended to the product catalog editor (2026-09-08, owner request, `cb854ce`)** | The guard above only knew about `state.stockEdits`. The editor's `state.draft` had no saved-vs-draft distinction at all to diff against — every field listener (`bindEditorEvents()`'s generic `[data-field]` handler, `syncNestedFromDom()` for colours/items/specs) writes straight into `state.draft` in place. New `markDraftSaved()` snapshots it (`structuredClone`) at every point it's set from a fresh load or a real save: `openNew()`, `openEditor()`, and the end of a new `saveProductDraft()` (extracted from the `#save-product` click handler, same "return true/false so the guard can await a real result" reasoning as `saveStockEdits()`) plus `saveFilament()`/`saveOneColour()`/`saveOneItem()` (all three fully refetch and overwrite `state.draft` from the server already, so marking clean at that point is accurate, not a new behavior). `hasUnsavedChanges()` diffs `state.draft` against the snapshot via `JSON.stringify` when `state.route === 'editor'`. Two real gaps found and fixed while wiring this in: the "← Back to catalog" button switched routes without ever going through the shared nav-btn click dispatcher the guard was hooked into (it's arguably the button MOST likely to hit this exact case — mid-edit, heading back to the list — so it got its own explicit `guardNavigation()` call); and deleting a product left a stale `state.draft`/`draftSaved` mismatch that would have wrongly guarded the very next navigation, fixed by clearing both on delete. Verified live: typing in an item's Name field and clicking Dashboard (or Back to catalog) triggers the same Stay/Discard/Save modal, Discard correctly leaves the record untouched (confirmed via a fresh `/api/inventory` fetch both times), and Save persists the edit and completes the navigation in one action. |
| **Unsaved-changes guard extended to Expenses + Settings; admin login by username or email (2026-09-08, owner request, `8def92c`)** | Two more pages wired into the same guard, plus an unrelated login change requested in the same session. **Expenses**: `state.editingExpense` (already a live-mutated form object, same shape as the editor's `state.draft`) gets a `state.editingExpenseSaved` snapshot via a new `markExpenseSaved()`, called from both "+ Expense" and the row Edit button; `saveExpenseForm()` extracted from `#save-expense` the same way `saveProductDraft()` was. **Settings**: architecturally different — its ~18 sections (`SETTINGS_SECTIONS`) each read straight from the DOM at their own scoped Save click (`scopedSettingFieldsPatch()`), with no per-section (or whole-page) JS object mirroring form state to diff against. Rather than build one, a single boolean `state.settingsDirty` is set by one delegated `input`/`change` listener wired ONCE onto the static `#view-settings` container (in `bindChrome()`, since only that container element — not its innerHTML — survives every `renderSettings()` re-render) and cleared at the top of `renderSettings()` itself, which already runs both on a fresh page load AND after every scoped save succeeds (a save's own re-render already wipes any other section's un-saved DOM edits today, so clearing the flag there matches what the screen now actually shows — not a new behavior, just correctly reflecting the existing one). Because Settings has no single action the guard could trigger, `showUnsavedChangesModal()` gained an `allowSave` option: false for Settings omits the Save button entirely and swaps the message to "go back and use that section's own Save button" instead of silently offering a Save that couldn't do anything. **Admin login by username or email**: new nullable `admins.email` column (`server/db.js`'s `ensureAdminEmailColumn` — a partial `UNIQUE` index over `email COLLATE NOCASE WHERE email IS NOT NULL AND email <> ''`, since SQLite's `ALTER TABLE` can't add a `UNIQUE` constraint directly, and a plain one would reject every admin past the first for sharing `NULL`). `verifyLogin(identifier, password)` now matches `username = ? OR (email IS NOT NULL AND LOWER(email) = LOWER(?))` and returns the account's stored `username`, never the raw `identifier` — a real bug caught before it shipped: the original code returned `{id: row.id, username}` using the *input* variable, which would have recorded the typed email as the session/audit-log username for every email-login. New `PATCH /api/admins/:id/email` route (`updateAdminEmail()`, same friendly-pre-check-before-INSERT pattern `createAdmin`'s username check already uses) + a Login Email field per row in Settings → Admin Accounts (every existing admin predates the column, so it's opt-in, set via its own small Save button) and on the Add Admin form. Login form label changed from "Username" to "Username or Email". 9 new tests (481/481 total): 4 in `admins.test.js` (case-insensitive email match, real-username-returned-not-email, duplicate-email rejected, two `NULL` emails don't collide, `updateAdminEmail` set/clear/reject-duplicate/self-reset-is-fine), 1 route-level in `index.test.js` covering the full PATCH → login-by-email (mixed case) → can't-steal-another-admin's-email chain. Verified live end-to-end: set a real email on the production "Admin" account via the Settings UI, logged in with it in uppercase (`ADMINTEST@EXAMPLE.COM`) — succeeded, and the audit log's `login_success` row correctly shows `username: "Admin"`, never the email — then cleared the email again, leaving production in its original state. Also confirmed both new guards live: Expenses (dirty supplier field → modal → Discard → no expense created) and Settings (dirty email field → modal with no Save button, only Stay/Discard, exactly as designed). |
| **Made-to-order + partial real stock — owner decision: manual toggle is enough (2026-09-08)** | Owner asked how to handle an item that's normally made-to-order but currently has a few real printed units sitting on the shelf — the `madeToOrder` flag (added the same day, see the manufacturing-cost row above) is one boolean per item, not per-unit, so it can't represent "3 of these 5 are real stock, 2 are just accept-orders capacity" simultaneously. Proposed the existing toggle as the practical answer rather than building anything new: keep an item on **Made to order** with `stockQty` set to whatever order volume the owner is willing to accept (not real inventory); when a real batch is actually printed and shelved, flip it to **Stocked** and set `stockQty` to the real physical count — Stock Value then counts it at manufacturing cost like any other real-stock item; once that batch sells through (`stockQty` hits 0), flip back to Made to order and reset `stockQty` to the normal accept-orders number. **Owner confirmed this manual workflow is sufficient for now** — explicitly declined a proposed richer feature (a real separate "units actually on hand" count distinct from "capacity accepted for made-to-order orders"). No code change; this row exists so a future session doesn't rebuild the toggle-workflow explanation from scratch or assume the simple model was an oversight rather than a deliberate, owner-confirmed choice. Revisit only if the owner raises it again — e.g. once they're routinely holding real stock on enough items that manually toggling starts to feel like the wrong shape. |
| **Stock Management Selling Price silently rounded away cents (2026-09-08 bugfix, `24455f9`)** | Owner report, caught while checking the new Manufacturing Cost formatting: category items' Selling Price showed as a whole number ("R450") even when the real stored price had cents ("R450.50"). Root cause: `server/inventory.js`'s local `parseRand()` (used only by `listInventory()`'s category-item branch, reading `item.price` — a formatted string like `"R450.50"` in `catalog.json`) wrapped its parsed value in `Math.round()`, discarding cents on every read; filament colours never had this bug since `price_rand` is a raw SQLite `REAL` column with nothing rounding it. The write path (`updateCategoryItemStock`'s `item.price = formatRand(...)`) already preserved cents correctly — so the bug was read-only until the Stock Management grid's own "Save Changes" button round-tripped the rounded display value back through the same write path, at which point the real cents were permanently overwritten with the rounded whole-rand number. Fixed by dropping the `Math.round()`. Same commit also gave the Selling Price input the `.rand-input` R-prefix and `.toFixed(2)` display the Buying Price/Manufacturing Cost columns already had (`step="1"` → `step="0.01"`), closing the last inconsistently-formatted money field in that grid. 1 new regression test (`inventory.test.js`), 482/482. Live-verified via the "PLA — PLA-White" row: input now shows `100.00` with the R prefix. |
| **Landrover manufacturing costs bulk-set to 30% of selling price (2026-09-08, owner request, data-only)** | Owner asked for every Landrover part's `manufacturingCost` calculated and captured as 30% of its `price` (a placeholder margin model, same spirit as the earlier flat-R180 filament buying-price bulk-set — real per-part costing can replace these over time via the same Stock Management field). All 194 Landrover category items had clean numeric prices (no `POA`/non-numeric ones to skip). Applied via a scripted `PUT /api/inventory` bulk call (`manufacturingCost: Math.round(price * 0.30 * 100) / 100` per item, same real authenticated-session API the admin UI itself uses — not a direct DB write) rather than a code change, since it's a one-time data operation, not a new feature. 194/194 rows updated with zero failures; spot-verified afterward that every row's `manufacturingCost` matches `round(price × 0.30, 2)` exactly (e.g. R50 item → R15.00, R250 item → R75.00). |
| **Expenses "Paid Via" renamed to "Paid From" (2026-09-08, owner request, `14bc473`)** | Label-only rename: the Expenses form field, the Expenses table column header, Settings → Expenses' payment-method configurable-list help text, and Financial Overview's per-account breakdown section header ("Per card / account" → "Per Paid From account"). Underlying field/setting names (`paymentMethod`, `expensePaymentMethods`, `data.paymentMethods` from `getFinanceOverview`) are unchanged — this was purely display text, so no data migration and no risk to existing expense records. Confirmed while making this change (not new work, just verified against the owner's other two asks in the same message): expense **Categories** and **Paid From accounts** were already independently manageable configurable lists in Settings → Expenses (`expenseCategories`/`expensePaymentMethods`, both existing since the Expenses module itself shipped 2026-09-07), and Financial Overview's **per-account expense breakdown already existed** (the "Per Paid From account" panel, sitting right next to the category breakdown) — the owner's request read as if these might be missing, but they were already built; only the "Paid Via" wording needed to change to match. Live-verified: Expenses table shows "PAID FROM" column header, Financial Overview shows "Per Paid From account" with real data (Revolving Acc R30,670, Linandi Nedcard R28,000, Johan FNB Card R10,000, Cheque ACC R9,720, Absa Credit Card R178.99). |
| **Sidebar nav group "Expenses" renamed to "Financial" (2026-09-08, owner request, `046b8e1`)** | Label-only: `admin/index.html`'s `.nav-group-label` and `admin.js`'s `titles` map (the eyebrow breadcrumb above each page's `<h1>`) both changed from "Expenses" to "Financial" for the group holding Expenses/Financial Overview/Stock Value — done specifically to make room for the Advances & Repayments page added in the same session (see below), which didn't fit under an "Expenses" heading. Route keys (`expenses`, `finance-overview`, `stock-value`) and every underlying field/table name are unchanged. |
| **Advances & Repayments — owner advances via Paid From accounts (2026-09-08, owner request, `811f2f5`)** | Owner's framing: every expense is paid from one of the owner's own Paid From accounts (cards/personal accounts) — that's money advanced INTO the business, not business capital, and until it's paid back the business isn't genuinely profitable, it's just spending advances. Design confirmed with the owner before building (two quick choices): every expense counts as an advance (not a subset flagged some other way), and repayments are a manual ledger the owner logs when money actually moves (not auto-derived from profit). **Key design decision**: the "Advanced" side of the balance is never stored anywhere new — it's computed live, all-time, as `SUM(expense_invoices.total) GROUP BY payment_method` (`server/account-repayments.js`'s `getAdvancesSummary()`, built on the existing `listExpenses()`), so there is exactly one source of truth for what was actually spent and nothing that could ever drift out of sync with the real expense records. Deliberately **not windowed to 12 months** the way Financial Overview's own `paymentMethods` breakdown is (that one exists purely to feed a 12-month trend chart) — an advance from over a year ago is still owed, so a windowed sum would be silently wrong here. Only the **repayment side** is a new table, `account_repayments` (id, account, amount, repaid_date, notes) — a plain ledger, CRUD via `POST/PUT/DELETE /api/account-repayments` (audited under the existing `SETTINGS_UPDATED` bucket, same convention Expenses itself already uses rather than adding a new enum value). `GET /api/account-repayments` returns both the repayment list AND the computed summary (`{account, advanced, repaid, outstanding}` per account, plus totals) in one call, since the admin page always needs both together. Accounts are the **union** of "ever had an expense" and "ever had a repayment" — a fully-repaid account (outstanding back to 0) still shows up rather than silently disappearing once its balance clears, and a repayment logged against an account with zero expenses (edge case, e.g. a typo'd account name) computes a negative outstanding rather than crashing, so a data-entry mistake is visible instead of hidden. New **Financial → Advances & Repayments** page (`admin/admin.js`): stat cards (Total advanced / Total repaid / Outstanding), a per-account breakdown table, a "Log a repayment" form (account dropdown from the existing `expensePaymentMethods` configurable list, R-prefixed amount, date, notes) mirroring Expenses' exact form-with-a-saved-snapshot shape, and a sortable repayment history table with edit/delete. Financial Overview gained one more stat card, "Outstanding advances", fed by the same `getAdvancesSummary()` call (`GET /api/finance/overview` now also returns `advancesOutstanding`). Wired into the existing unsaved-changes nav guard exactly like Expenses was (`state.editingRepayment`/`state.editingRepaymentSaved`, `markRepaymentSaved()`, `saveRepaymentForm()` returning true/false for the guard's Save choice) — this is now the 5th page the guard covers. 8 new tests (490/490 total): 7 unit-level in `account-repayments.test.js` (validation, full CRUD, the all-time-vs-windowed advances math specifically proven with an expense dated 2020 to rule out any 12-month leakage, a fully-repaid account still listed, an orphan repayment not crashing), 1 route-level in `index.test.js` covering the real create-expense → log-repayment → Financial-Overview-reflects-it → edit → delete loop through the actual API (plus a bare 401 check). `db.test.js`'s schema-completeness list updated for the new table. Live-verified on production: the per-account table matched the real Expenses grand total exactly (R78,568.99 across 5 accounts) before any repayment existed; logged a real R100 test repayment against Absa Credit Card through the actual UI form, watched Outstanding drop to R78,468.99 on both this page AND Financial Overview's new stat card in the same figure, then deleted it via the UI's own Delete button, leaving production at R0 repaid / clean. |
| **"Move To" — move a filament colour between filament types (2026-09-09, owner request, `a344a95`)** | Owner had rolls captured under the wrong filament type (e.g. a PETG roll added under PLA) — previously only fixable by deleting and recreating the colour, losing its SKU history, stock count, and photos. New button next to each colour's own **Remove** on the filament editor (`admin/admin.js`'s colour row-card, `[data-move-colour]`, hidden for a not-yet-saved `_isNew` colour same as the gallery panel already is) opens a small on-demand modal (`showMoveColourModal()`, same overlay-built-on-click pattern as `showUnsavedChangesModal()`) listing every OTHER filament type to move to. Confirms via a new `POST /api/filaments/:filamentId/colours/:colourId/move` route → `moveColourToFilament()` in `server/filaments.js`: a plain `UPDATE filament_colours SET filament_type_id = ?` — nothing else about the row changes, since `sku` is already globally `UNIQUE` across every colour regardless of type (no collision risk, no SKU rewrite needed). Same active-special guard `deleteColour()` already has: refuses to move a colour with `special_status = 'active'` (its pricing/countdown context is tied to the type it was started on), while the special's own *base* colour still moves freely — moving the base doesn't affect an in-flight special at all, since the special row is a fully independent `filament_colours` record referencing the base only by id. The route returns the SOURCE filament (now missing that colour) so the editor updates in place without a full reload; the client also calls `markDraftSaved()` on success, same as every other real save on this page, so the unsaved-changes guard doesn't misfire afterward. 4 new tests (494/494 total): 3 unit-level in `filaments.test.js` (sku/stock/price survive the move intact, rejects same-type/unknown-type/unknown-colour, refuses an active special but allows its base colour), 1 route-level in `index.test.js` covering the real move → gone-from-source → present-on-target chain plus a 401 check. Live-verified on production: moved PETG Eco's only colour ("Black", `SA-F-PETGECO-A02-100175PG`) into ASA through the actual UI (button → modal → select → Move), confirmed it landed on ASA with its stock/SKU intact and disappeared from PETG Eco, then moved it back the same way and confirmed both filament types matched their original colour lists exactly (PETG Eco's stockQty back to its real value of 0) — production left exactly as found. |
| **Row-card action-button standard established; Product Catalog table column widths fixed (2026-09-09, owner request, `0b16388` + `21eeef9` + `4c7fe29`)** | Three-part cleanup pass, triggered by the owner asking to make the new Move To button match Save Roll's layout. **The standard** (documented inline in `admin.js` above both `colours-list` and `items-list`): one `row-card-actions` line = `#N label`, then buttons safe-to-dangerous left to right (`btn-primary` save action(s) first, any neutral `btn` action next, `btn-danger` destructive last) — gallery/upload/video UI always gets its OWN separate `row-card-actions` line, never mixed into the button line. **Colour row-card** (`0b16388`): was split across two lines (gallery+Move To+Remove on one, Save Roll alone right-aligned on another) — now one line (`#N name` → Save roll → Move To → Remove), gallery on its own line below, matching the category-item row-card's shape exactly (that one was already correct and became the reference). **Item row-card video panel** (`21eeef9`): the main Save item/Remove line already matched, but the video sub-panel didn't — Remove video sat in the middle (View video, Remove video, Add/Replace video) instead of last; reordered to View video → Add/Replace video → Remove video, and Remove video now only renders when a video actually exists (previously grouped with View video under the same guard, so this was already effectively correct — the reorder is what actually changed). **Product Catalog table columns** (`4c7fe29`, separate but same-day owner report): filament/Toys/GWM/Landrover rows read as "very condensed to the left" despite `table.catalog` already being full-width — the browser's auto table-layout gave short-content columns (Kind, Status, Updated: a word, a badge, a date) the same leftover-width share as content-heavy ones, so each row's real information sat bunched at the left of oversized cells with dead space stretching off to the right. Fixed with `table-layout: fixed` + explicit percentages (Product 28% / Kind 10% / Status 10% / Details 32% / Updated 15% / edit 5%), scoped to `#view-catalog table.catalog` specifically so every other page reusing the shared `.catalog` table class (Orders, Clients, Stock Management, ...) keeps its own auto-sized columns untouched. All three verified live: colour/item row buttons confirmed via DOM inspection on real production rows (ASA colour, Toys item with no video); catalog table columns measured before (341/197/209/309/351/142px at 1920px) and after (434/155/155/496/232/77px) on the real deployed CSS, both nested (Landrover) and top-level sections checked. No server changes for any of the three; no new tests (pure markup/CSS), full suite unaffected at 494/494. |
| **"Featured on Homepage Cues" wired to actually gate a category's sidebar/page/purchasability; stale category pages now pruned (2026-09-09, owner request, `94b8162` + `4b6d996`)** | Owner report: unticking Featured on a category did nothing — it stayed in the sidebar. Investigation found the field was **completely dead** — `p.featured`/`item.featured` round-tripped through save/load (DB column, catalog.json field, even had its own tests) but was never READ anywhere the storefront actually renders: not `generate-pages.mjs`, not `nav.js`, not `orders.js`. Presented the finding and three options to the owner rather than guessing; **owner chose: repurpose it to mean what they expected — unfeaturing a category takes it fully offline** (sidebar + public page + purchasability), a second way to reach the same end state Draft status already had for dynamic categories. **Critical pre-deploy step, done live on production before any of this code shipped**: every existing category (toys/homeware/weed-shoppe/phones/gwm/landrover) already had `featured: false` — the field's original always-false dead state — so shipping the gate naively would have taken every single category offline simultaneously on deploy. Bulk-set all six to `featured: true` via the real authenticated API first, verified all six read back `true`, **only then** deployed the code that makes the field matter. Four coordinated enforcement points, each independently real (not just one gate with everything else trusting it): `server/export.js` now includes `featured` in the public categories export (previously omitted entirely — the generator could never have seen it even if it had tried to read it); `scripts/generate-pages.mjs`'s `categoryPages` loop skips page generation for `featured === false` OR `status === 'draft'`, applied in the one place every category (core three, car-part brands, AND dynamic categories) already funnels through — draft's OWN filter used to happen earlier, before the loop, which is exactly why draft categories had the same latent bug described next; `src/data/site.js`'s `CORE_CATEGORY_NAV` and `CAR_BRANDS_NAV` both gated by a new shared `categoryIsLive()` helper (previously the core three had **no gate of any kind** on their nav links, and `CAR_BRANDS_NAV` only checked the category existed, never its status) — closing a separate pre-existing inconsistency found along the way, not just adding the new featured check; `server/orders.js`'s `resolveProductSnapshot` category branch rejects the order outright when the category is draft or unfeatured — the real backstop, proven live by temporarily marking a listed+available+in-stock item's category unfeatured and confirming `POST /api/checkout` returned `400 "Product no longer available"` even though every item-level flag said it was purchasable; a stale cached page or a bookmarked direct link cannot complete a sale for something the storefront no longer shows anywhere. **Second bug found live during verification, fixed same session** (`4b6d996`): the generator correctly STOPPED regenerating an unfeatured category's page, but never deleted the copy already on disk from when it was live — proved by unfeaturing `weed-shoppe.html` and finding it still served 200 with stale placeholder content. New `removeStaleCategoryPage()` deletes it (same `pruneStaleDetailPages()` pattern this file already used for item/colour detail pages), and the draft-status candidate-list pre-filter was folded into the same in-loop check so drafting a category gets the identical cleanup, closing the same class of gap there too — this is a **net behavior change**: the core three and car-part brand pages can now also go offline via Draft status, something they had never been capable of before, current production data unaffected only because nothing is currently draft. Admin UI: the "Featured" checkbox's label and a new inline hint (category kind only — still a genuine no-op for filament, deliberately out of scope) now say what unticking it actually does. 5 new tests across this pair of commits (495/495 total): `orders.test.js` (draft and unfeatured both reject checkout resolution; published+featured resolves normally, new `withTempCwd` isolation added to that file since category products live in catalog.json, not SQLite), `generate-pages.test.js` extended with an unfeatured-but-published case alongside the existing draft-shoppe one. **Fully live-verified end to end, twice** (once catching the stale-page bug, once confirming the fix): unfeatured Weed Shop → sidebar link gone, `weed-shoppe.html` → 404, checkout rejected with the item still individually listed/available/in-stock → re-featured → page regenerated (200) and back in the sidebar → confirmed every category's `featured`/`status`/the real item's `listed`/`available`/`stockQty` matched their exact original values afterward, production left clean. |
| **3 placeholder stub items deleted from Toys/Home & School (2026-09-09, owner request, data-only)** | Follow-up to the Featured-gating work above: owner asked for a list of every category item still `listed: false` across the whole catalog. Found 4 — "Toys piece 2", "Toys piece 3" (Toys), "Homeware piece 3" (Home & School), and the real "8x10 Tube Tray" (Weed Shop). The three "piece N" ones were leftover fixture stubs — no SKU, no price, the generator's own `item.name || \`${label} piece\`` fallback naming from having no real name ever set — not real products, just never-filled-in placeholder rows. Owner confirmed deleting the three stubs; Weed Shop's real item was left untouched (still a genuine pending decision, not a stub). Deleted via the real `DELETE /api/products/:productId/items/:itemId` route (audited, auto-publishes) — no code change, no deploy needed, already live the moment the calls ran. Verified afterward: catalog-wide unlisted-item query returns only the one real Weed Shop item. |
| **New "Advertise" admin section: Adverts, Platform Rules, Calendar (2026-09-09, owner request, `236dfdc`)** | Owner asked for a way to plan and track social/email adverts across platforms. Brainstormed and clarified with the owner before building (3 questions): caption field on adverts — yes; Calendar layout — agenda list over a month grid; Platform list — **configurable**, not the fixed 7 the owner had actually named (Facebook, Facebook Groups, TikTok, Direct Email, Direct WhatsApp, Instagram, WhatsApp Groups) — those 7 are only the one-time seed, not a hardcoded set. New dedicated module `server/advertising.js` (not the generic `{id,name,active}` configurable-list panel, since platforms need an extra `hasGroups` flag and a nested sub-entity) covers three related entities: **Platforms** (owner-configurable, retire-via-`active` + refuse hard-delete while referenced by a scheduled advert, same guard pattern as `deleteAdmin`/`deleteColour`), **Platform Groups** (only for platforms flagged `hasGroups` — seeded true for Facebook Groups and WhatsApp Groups; Group Name, Allowed Posting Days as a `Mon..Sun` set stored as a normalized comma string, Notes), and **Adverts** (platform, optional caption, one uploaded image via a new `uploadAdvertImage`/`deleteAdvertImage` pair in `uploads.js` mirroring the category-item image pattern exactly, publish date/time, and a duration-in-days that's inclusive of the start day — a 1-day advert starts and ends the same day). 3 new tables (`advert_platforms`, `advert_platform_groups`, `adverts`) + idempotent `seedAdvertPlatforms()` boot seed for the 7 named platforms, in the owner's given order. **Bug caught by the unit tests, fixed before shipping**: `computeEndDate()`'s first draft parsed the publish date as local midnight, did day-arithmetic, then formatted via `toISOString()` (UTC) — on this SAST (UTC+2) server that silently shifted every computed end date back one full day; fixed to do the arithmetic in `Date.UTC` throughout so the local server timezone can never cross a date boundary. New admin nav group **Advertise** (`admin/index.html`, between Financial and Settings) with three pages (`admin/admin.js`): **Adverts** — sortable list + create/edit form, image upload gated on the advert already being saved (same "save first, then upload" shape as Testimonials' photo field, since the upload route needs a real advert id); **Platform Rules** — one card per platform (Edit/Delete + Notes), and for `hasGroups` platforms a nested Group Name/Allowed Days/Notes table with its own inline add/edit form; **Calendar** — an agenda list from 7 days back to 21 days ahead of today (owner's exact window), grouped by day, each advert shown on every day its run actually covers (not just its start day) so a multi-day advert doesn't look like it only ran once, with a "Today" marker even on a day with nothing scheduled so the reader stays oriented in the list. All three routes sit under the existing `MARKETING_UPDATED` audit bucket (same convention as Newsletter/WhatsApp/Potential Market — reusing an existing broad bucket rather than adding a new enum value). 7 new unit tests in `server/advertising.test.js` (502/502 total): the exact 7-platform seed order/flags, platform CRUD + the referenced-platform delete guard, group CRUD including the day-set normalization dropping an invalid day name, advert CRUD, the inclusive end-date math, and the calendar's from/to overlap query (proven with an advert that starts before the window, one that spans past it, and one well outside it). `db.test.js`'s schema-completeness list updated for the 3 new tables. Live-verified on production end to end through the real UI: scheduled a 3-day Facebook advert dated the day of shipping and confirmed the computed end date landed 2 days later (not 1, the exact class of bug the timezone fix caught); the Calendar page showed it correctly on all 3 days of its run with a "Today" marker on day one; added a real group ("3D Printing Enthusiasts SA") under Facebook Groups and confirmed it listed correctly; both the test advert and test group were deleted afterward via direct authenticated API calls (the browser tool's confirm() dialogs are suppressed in this environment, so the UI's own Delete buttons couldn't be click-tested end-to-end, only their handlers' wiring) — production left with all 7 platforms and zero adverts/groups, exactly as before. |
| **Dropship (Esquire) module — real supplier feed, curated margin-priced listings (2026-09-10, owner request)** | Owner is registered with esquire.co.za (computer equipment + some homeware), supplies dropshipping, wants a 10% default markup he can adjust himself. Owner supplied the live feed URL with embedded credentials (`u=`/`p=` query params) — treated as a secret throughout: stored only in `settings.esquireFeedUrl` (server-side setting, never in public export), never logged, never committed in plaintext anywhere. **Key empirical finding before any design**: the feed's own `m=` query param is a server-side margin — verified by comparing `m=10` vs `m=0` responses for 15 real SKUs and finding an exact ×1.10 ratio every time. This codebase always forces `m=0` via `buildFeedUrl()` (`server/esquire.js`) to get true wholesale cost, then computes its own selling price via `computeSellingPrice(cost, marginPercent)` — the supplier's margin knob is never relied on, so the owner's own percentage is the only one that matters. Clarified design with the owner before building (4 questions): fulfillment manual (owner places the actual supplier order by hand per sale, no auto-ordering); shipping a flat fee (not weight/distance calculated); scope a **curated subset** the owner picks per item, not the whole feed dumped in; margin adjustable by the owner per listing, not fixed at 10% forever. Shipped with **R99 flat dropship fee, 10% default margin** (`settings.esquireFlatShippingFee`, `settings.esquireDefaultMarginPercent`) — a mixed cart (dropship + real stock) adds both the normal courier fee and the flat dropship fee (owner's explicit choice, "Add both fees"). New module `server/esquire.js`: `fast-xml-parser` parses the feed (`cdataPropName: '__cdata'`; the feed double-escapes HTML entities inside its own CDATA, un-escaped once via a `cdata()` helper); `syncEsquireProducts()` wholesale-replaces the `esquire_products` cache table (missing codes marked unavailable, never deleted, so history/pricing isn't lost on a transient supplier-side removal); `createDropshipListing()`/`updateDropshipListing()`/`deleteDropshipListing()` manage `dropship_listings` rows, each backing a real category-product item (`dropship:true, esquireProductCode, marginPercent` on the item) via the shared `normalizeItem()`; `resyncDropshipListings()` refreshes cost/price/stock for active listings on each sync and auto-delists when the supplier drops availability, while preserving any owner-uploaded photo. **No real quantity in the feed** (only Yes/No availability), so dropship items get a nominal `DROPSHIP_NOMINAL_STOCK = 999` ceiling — not a promise of 999 real units, just lets the existing reserve-at-order-creation stock machinery run unmodified; real oversell risk against the supplier is inherent to dropshipping and is an operational, not a technical, concern. New admin section **Dropship (Esquire)**: Browse Feed (search/filter the cached supplier catalogue, one-click "List this item" with a category + margin picker), Listings (active dropship items, edit margin, delete), Settings (feed URL, flat fee, default margin — all three newly added to PUT `/api/settings`'s write allowlist, since a new settings key silently failing to persist there is a recurring bug class in this codebase, 4th occurrence). Public-facing: `server/export.js` includes `dropship: item.dropship === true` on exported items; cart/checkout (`src/js/cart.js`, `cart-ui.js`, `checkout-entry.js`, `checkout.html`) thread the flag through and show the flat dropship fee at checkout; `server/orders.js` computes and stores `dropship_fee` per order. **Timezone-class bug avoided by reusing the Advertise feature's existing fix** — none introduced here. 17 new unit tests in `server/esquire.test.js` + route-level coverage in `index.test.js` (settings persistence, sync, listing CRUD, checkout fee calc). First real batch: 14 curated computer-accessory items imported and live-verified on production. |
| **Full Esquire catalogue import → 653-category sidebar overload → umbrella-category classifier fix (2026-09-10, owner request + owner-caught production issue)** | Owner asked to import "literally everything" remaining in the feed (an explicit, non-default choice over the recommended narrower scope) via `bulkImportRemainingProducts()` — loads the cached catalogue once, buckets every un-imported available product, single `saveCatalog()` call. First pass mapped each of the feed's ~653 raw Esquire categories 1:1 to a new storefront category, which immediately produced ~653 sidebar entries — flagged live by the owner ("the added Left menu items is too many") as unusable. Fixed with `classifyEsquireCategory()` + `ESQUIRE_CATEGORY_GROUPS`, an ordered keyword-matching classifier collapsing the raw categories into ~19 sensible umbrella groups (e.g. Computers & Laptops, Networking, Storage, Cables & Adapters, Mice/Keyboards/Peripherals, Home Appliances…) plus a `general-merchandise` fallback for anything unmatched. **Gap caught by the classifier's own unit test**: an expected `wireless-mouse` classification fell through to the fallback group — added a missing `computer-accessories-peripherals` group before shipping. `migrateGranularCategoriesToGroups()` regroups the already-created 1:1 categories into the new umbrella groups and deletes the now-empty old ones; safe to re-run. Distinguishing an **auto-created** (safe to migrate) from a **curated/hand-picked** listing uses a fingerprint check — `category_slug === slugify(the item's own raw Esquire category)` — rather than new schema, since a deliberately-chosen slug never coincidentally matches that pattern. Regression this exposed in the static generator: `removeStaleCategoryPage()` only pruned a category still a page-generation *candidate* that run (draft/unfeatured/missing data) — a category deleted **outright** (as the migration does to the old 1:1 categories) was never a candidate, so its stale `.html` kept serving 200s indefinitely. Fixed with a persistent manifest (`data/category-pages-manifest.json`, gitignored) diffed run-over-run in `scripts/generate-pages.mjs`, so any slug present in last run's manifest but absent this run gets its page removed regardless of why it disappeared. No baseline existed for the 652 pages that had already gone stale before the manifest existed — required a one-time manual cleanup script run directly on the VPS (slugify each of the 653 original raw category names, delete the matching root `.html` if not in the current valid-slugs set); removed exactly 652, matching the migration's own reported count. New test coverage: `generate-pages.test.js` (stale-category-deletion case), `esquire.test.js` (classifier keyword coverage, migration fingerprint safety). |
| **Production OOM crash during the full import's rebuild — 3-factor root cause, kill switch, swap fix (2026-09-10, live incident, owner-reported)** | Owner reported the site was down immediately after the full Esquire import ("It is clear the latest additions is crashing the site... Disable all the esquire items at a menu level and product level") — treated as an urgent stability directive, actioned before anything else. **Root cause, 3 independently compounding factors** (diagnosed via direct `journalctl` reading on the VPS, pinpointing the OOM-kill at vite's `transforming...` build stage, and repeated `ps aux` snapshots showing stacked builds): (1) **capacity** — the shared VPS (1.9GB RAM, zero swap at the time) can't fit vite's single-process multi-page build (one rollup entry per root `.html` file) once the page count reached ~4,800 after the full import; a solo build already peaked 1.4–1.7GB RSS even before the import. (2) **retry behavior** — `server/jobs.js`'s convention that every scheduled job "runs once on boot too" meant `startEsquireSyncJob` re-triggered a full rebuild on every restart, and systemd's auto-restart-on-crash turned one OOM kill into a self-sustaining crash loop with no backoff. (3) **concurrency** — `publishCatalog()` (in `server/index.js`, runs `syncPublicJson()` + `runGenerate()` + `runBuild()` after every catalog-mutating route) has no mutual exclusion; repeated restarts plus independent API calls each triggering it stacked up to 3 concurrent `vite build` processes observed simultaneously, thrashing swap hard enough that SSH and HTTPS both intermittently refused connections. **Immediate mitigation, per explicit owner instruction to build/test locally and hold deploy until VPS recovery was independently confirmed**: `disableAllDropshipItems()` (`server/esquire.js`) — a reversible kill switch, never a delete: every category product containing a `dropship:true` item gets `status:'draft'`, every such item gets `listed:false, available:false`, every `dropship_listings` row gets `active:0`. Wired to a "Kill Switch" panel in the admin Dropship section (`esquire-disable-all` button) and a `POST /api/esquire/disable-all` route. Verified VPS reachability (`curl`, `ssh`) before pushing, per the owner's explicit hold. **Infra fix**: added a 2GB swapfile on the VPS (`fallocate` + `mkswap` + `swapon` + `/etc/fstab` entry) so a single build no longer risks OOM. **Still an open architectural gap, offered to the owner but not yet confirmed**: no publish lock around `publishCatalog()`, and no boot-cooldown or removal of the "run once on boot" behavior for jobs that can trigger a heavy rebuild — both would need to ship before dropship items are safely re-enabled at the current page count. |
| **Storefront sidebar reorder — Car Parts to top and always-open, Our Story/Get in Touch to bottom (2026-09-10, owner request, no server changes)** | Owner asked to move Car Parts to the top of the sidebar and have it permanently expanded (previously an openable/closable `<details>` like Filament), and move Our Story + Get in Touch to the bottom. Pure `src/js/nav.js` change — `renderSidebar()` is the sole authoritative source of the storefront sidebar (the generator only emits an empty `<nav data-site-nav>` container; `mountNav()` fills it client-side on every page load), so no generator or server change was needed. Car Parts' `<details>` gained a bare `open` attribute (no longer conditional on `openGroups`/current-page state — always expanded regardless of what the visitor is looking at) and moved above Filament in source order; Our Story and Get in Touch links moved to the very end of the list, after My Account. |
| **In-House Filament force-delete override for print-job-referenced duplicates (2026-09-10, owner request, `server/in-house-filament.js` + `server/index.js`)** | Owner reported duplicate/mis-captured in-house filament rows (some genuinely captured in error) that couldn't be deleted — the existing `deleteInHouseFilament()` correctly refuses once a `print_job_filaments` row references the filament (a raw SQLite foreign-key error caught and turned into a clear message: "Cannot delete — this filament has been used in a logged print job."). Owner asked for an override. New `getInHouseFilamentUsage(id)` returns every logged print job referencing a filament (job id, item name, date, grams, cost) so the admin UI's confirm dialog can name exactly what's about to be lost, rather than a bare job count. New `forceDeleteInHouseFilament(id)` — deliberately a **separate function** from the normal delete, never a silent `force` flag bolted onto it, so this destructive path is its own distinct, auditable action: in one transaction, deletes every `print_job_filaments` row referencing the filament (capturing the count), then deletes the filament row itself. The print job's own row and its OTHER filament slots are untouched — only the one slot for the deleted filament disappears from that job's cost breakdown. New routes: `GET /api/in-house-filament/:id/usage`, `DELETE /api/in-house-filament/:id/force` (audited under `STOCK_UPDATED`, detail names the filament and how many jobs lost a cost line). Admin UI: `offerForceDeleteInHouseFilament()` wired into the existing delete handler's catch block — a normal delete attempt that hits the FK refusal now offers the force option inline, naming the affected jobs. 2 new tests in `in-house-filament.test.js` (refuse-then-force-delete round trip proving the job row and its other slots survive; zero-usage case). Live-tested via `window.confirm = () => true` monkey-patch (this environment's browser tool auto-suppresses native `confirm()`/`alert()` dialogs). |
| **Print Job Costing: per-colour Tower/Purge/Supports usage + per-printer power cost (2026-09-23, owner request, `af36aca`)** | Owner asked for space per colour to capture the grams/metres that go to the prime **tower**, **purge** and **supports**, and for electricity to be costed per printer. **Usage:** each filament slot on the form is now one compact line (owner choice over sub-rows/collapsible) — Model, Tower, Purge, Supports, each a g + m pair. Owner decision: all four are entered **per copy** and multiplied by `quantity` (tower/purge included, not once-per-plate). New `model_g`/`model_m`/`tower_g`/`tower_m`/`purge_g`/`purge_m`/`support_g`/`support_m` columns on `print_job_filaments` (guarded `ALTER`s in `ensurePrintJobColumns()`; pre-existing rows backfill 0 — their totals were never split). `grams`/`meters` stay the slot **total** (sum of the four), so stock decrement, filament cost and every other reader are unchanged. `readSlotRoles()` in `server/print-jobs.js` still accepts a flat `grams`/`meters` slot (treated as all-model) — keeps `scripts/import-historical-print-jobs.mjs` and older callers working. **Power:** new Settings list `printers` (`{id,name,watts,active}[]`, seeded **Bambu Lab P2S 150W** / **Snapmaker U1 200W** — rough placeholders for the owner to correct), edited as rows inside Settings → Print Job Costing Rates (replaces the single Printer Power Draw field), shape-guarded in `PUT /api/settings`. The job form gets a **Printer** picker (remembers the last one after logging); `computeJobCost()` resolves it and costs power as `hours × watts/1000 × electricityRate`. New `printer_id`/`printer_name`/`printer_watts` columns on `print_jobs` snapshot the printer at log time — re-rating/renaming a printer never rewrites history (same snapshot rule as `power_cost`). Unknown printer id → 400 error; retired (`active:false`) printers only drop out of the picker. No printer (import script) → legacy `printerPowerDraw` (kW) fallback. `electricityRate` default raised R3.85 → **R4.97** (a DB-saved value still overrides — production must be set in Settings if it ever saved 3.85). `printers` added to the export leak test's forbidden list (never public). 3 new tests. Deployed to production 2026-09-24 as **V1.54**. | 534/534 |
| **Print Job Costing: owner feedback round — m-before-g, Purge before Tower, per-colour Totals, Recommended vs Final price, attachments fix, full-width form (2026-09-24, owner request)** | Eight-point owner list. **Layout:** every g/m pair now reads **metres first** (form, per-colour Total, All-Filaments line, table's "Filament used (m / g)"); role order is **Model → Purge → Tower → Supports** (`PRINT_JOB_ROLES` order only — server `SLOT_ROLES` order is irrelevant to storage); each colour row gains a read-only **Total (m / g, Per Copy)** pair, updated live as you type (`grid-5`, new in `admin.css`, collapses to 2 cols ≤1100px and 1 col on mobile). The Log a Print Job panel lost its `max-width:900px` and now matches the table's full width (name + quantity share a row; prices get their own row). **Attachments bug (real, owner-reported):** a model file/photo chosen on the form was silently never uploaded if anything re-rendered the form before Log — Validate, picking a filament or printer, or + New Roll all rebuild the view, and a file `<input>`'s selection can't be restored. The `File` objects now live on the draft (`draft.modelFile`/`modelImage`, fields that already existed but were never used), the chosen filename is shown with Replace/Remove, and Log uploads from the draft. **Prices:** the admin-editable price formerly labelled "Final Selling Price" is now **Recommended Selling Price** (new `print_jobs.recommended_selling_price`, backfilled once at column creation from `COALESCE(final_selling_price, selling_price)` so no existing job or listing changes), and a new **Final Selling Price** sits beside it. Blank Recommended → Minimum; blank Final → Recommended (`positiveOr()` in `print-jobs.js`). **Final remains what "List for sale" uses.** Both are inline-editable in the table (`pj-price-cell`, one PATCH per field); PATCH accepts either independently, audit detail logs both; the historical import script writes both. **Also fixed:** `.main` had CSS-grid `min-width:auto`, so every admin page ran ~15px past the viewport (Print Jobs ~48px) with a horizontal scrollbar — `min-width:0`. Verified in-browser via a throwaway harness page that stubbed `fetch` so the real `admin.js` rendered without a login (deleted after): totals 19.60 m / 58.7 g correct, file survived Validate and uploaded to `/api/print-jobs/:id/file`, payload carried all 8 role values + both prices. 2 new tests. Deployed to production 2026-09-24 as **V1.55**. | 536/536 |
| **Todos #168/#169/#170 + print-job grand-total row (2026-09-24, owner request)** | **#170 Edit / re-estimate a print job:** new **Edit** button per row loads the job back into the form (`printJobToDraft()` — whole-batch slot amounts ÷ quantity back to per-copy; pre-breakdown slots put their total into Model; archived rolls the job uses stay selectable; a retired/removed printer falls back to the first active one), heading becomes "Edit Print Job — name", button "Save changes & re-cost" + Cancel edit. New `editPrintJob()` + `PUT /api/print-jobs/:id` (audited; `PATCH` stays the inline status/price update): recomputes everything with today's settings/roll prices; in ONE transaction hands back the old slots' usage (`MAX(0, used - old)` — clamped because historically-imported jobs never charged stock) then writes the new slots via the shared `writeJobSlots()` (extracted from `createPrintJob`), so in-house stock moves only by the difference. Keeps `created_at`, attachments, List-for-sale link; stock warnings count the job's own handed-back grams as available. Jobs with >4 filaments (some historical imports) refuse edit with a message. **#168 tracking-number confirmation:** `PUT /api/orders/:id/tracking` now returns `shippedEmail: {sent, to}` or `{sent:false, reason}` (cleared / already emailed on first save / no customer email / send failed); Order detail shows a persistent confirmation panel ("Tracking number X saved… 'Order shipped' email sent to …" or why not) instead of a 3-second "Order updated" toast. Note: the customer email is the **Order shipped** template (the one carrying the tracking number), sent only the first time a number is set — unchanged behaviour, now visible. **#169 WhatsApp enquiry text:** "…contacting you from your new 3D site." → "…from your 3D site." — the live value is the `whatsapp` setting (URL-encoded, which is why a plain-text search missed it), updated in the production DB at deploy; fallbacks in `settings-defaults.js`, `generate-pages.mjs`, `src/data/site.js`, `src/data/settings.json`, `index.html`, `get-in-touch.html` updated too; generated pages pick it up on regenerate. **Grand-total row:** under Filament 4, per-role (Model/Purge/Tower/Supports) and Grand Total m / g summed across all colours, live — replaces the one-line "All Filaments" summary. Verified in-browser via the fetch-stub harness (edit prefill, PUT payload, grand totals, tracking banner). 4 new tests. | 538/538 |
...
| recommended_selling_price | REAL NULL | Admin-editable **Recommended Selling Price** (2026-09-24) — defaults to `selling_price` at creation if not supplied. Backfilled once from `final_selling_price` when the column was added |
| final_selling_price | REAL | Admin-editable **Final Selling Price** — defaults to `recommended_selling_price` at creation if not supplied. What actually gets used as the price if/when this job is listed for sale |
...
| created_at | TEXT | |
| printer_id, printer_name, printer_watts | TEXT/TEXT/REAL NULL | The printer the job ran on, snapshotted from Settings `printers` at log time (2026-09-23). `NULL` on jobs logged before per-printer power existed (costed with the legacy single `printerPowerDraw`) |
...
| grams, meters, cost | REAL — `grams`/`meters` are the slot TOTAL for the whole batch (sum of the four roles below × quantity) |
| model_g, model_m, tower_g, tower_m, purge_g, purge_m, support_g, support_m | REAL DEFAULT 0 — per-role breakdown for the whole batch (2026-09-23). 0 on rows logged before this existed |
| slot_order | INTEGER |
...
| PUT | `/api/print-jobs/:id` | Admin | Full edit / re-estimate (same body as create). Recomputes cost; adjusts in-house stock by the difference (old usage handed back, clamped at 0, new usage charged) in one transaction. Keeps created_at, attachments, listing link |
| PATCH/DELETE | `/api/print-jobs/:id` | Admin | Update (status, recommendedSellingPrice, finalSellingPrice — each optional/independent) / delete |
...
| PATCH | `/api/orders/:id/collected` | Admin | Set/clear the Collected tick (`collected_at`) |
...
```mermaid
flowchart TD
    A[Admin selects up to 4<br/>in-house filaments + per colour:<br/>model/tower/purge/supports g+m] --> B[Pick printer + enter time inputs:<br/>print time, design/setup/post-processing hours]
    B --> C[Enter markup %]
...
- **Business rules:**
  ...
  - Status transitions: `pending_payment → paid → shipped → completed`, plus `cancelled` reachable two ways (the automatic 7-day stale-order job was removed 2026-09-03 per owner instruction): explicit admin action (any status), or the order's own client, self-service, from the account page's "Cancel" button (`cancelOrderByClient` — only while still `pending_payment`, ownership-checked). Both cancellation paths release the order's reserved stock back (`restoreStockForOrder`, guarded against double-restoring an already-cancelled order) and email `settings.orderNotificationEmail` (`sendOrderCancelledNotificationEmail`) so the business owner sees every cancellation regardless of which path triggered it.
...
- **Recommended Selling Price** (`recommendedSellingPrice`) is admin-editable, defaults to the Minimum Selling Price at creation if left blank.
- **Final Selling Price** (`finalSellingPrice`) is admin-editable, defaults to the Recommended Selling Price at creation if left blank. Both can be changed any time afterward (inline in the table, blur-saves). **Final** is the price actually used if the job is listed for sale.
...
|---|---|---|
| itemName | Yes | |
| filament slots (up to 4) | ≥1 | Each: in-house filament pick + Model / Purge / Tower / Supports, each metres + grams, per copy; read-only Total (m / g) = sum |
| printerId | No (form always sends one) | From Settings `printers`; drives power draw. Omitted → legacy `printerPowerDraw` |
| printTimeMinutes | Yes | |
| designHours, setupHours, postProcessingHours | No, default 0 | |
| markupPct | No, default from settings | |
| recommendedSellingPrice | No, default = Minimum Selling Price | |
| finalSellingPrice | No, default = Recommended Selling Price | Used by "List for sale" |
| model file / reference photo | No | Held on the form draft (survives Validate/re-renders), uploaded right after the job is logged |

**Calculation:** `filamentCost` (per-slot cost from `in_house_filament.cost_per_roll_rand` proportional to grams/metres used) + `powerCost` (print hours × quantity × chosen printer's watts ÷ 1000 × electricityRate; legacy `printerPowerDraw` kW if no printer) + `labourCost` ((design+setup+postProcessing hours) × their respective rates) → `runningCost` (× runningCostsPct) → `totalCost` → `markupAmount` (× markupPct) → `sellingPrice`.
