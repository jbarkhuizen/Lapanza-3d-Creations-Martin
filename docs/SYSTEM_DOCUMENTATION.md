...
| **Orders fulfilment tracking (2026-09-03, owner request)** | The admin Orders list now shows the invoice number and three per-order tick boxes: Collected, Shipped, Finalized. Collected records the physical handover in a new `orders.collected_at` column (independent of status -- courier orders can also be collected; `PATCH /api/orders/:id/collected`, audit-logged). Shipped and Finalized are views onto the existing status machine: ticking Shipped sets status `shipped` (untick returns to `paid`), ticking Finalized sets `completed` (untick returns to `shipped`) -- both through the audited `PUT /api/orders/:id/status` route, so cancellation restock and payment-status lockstep still apply. Boxes are disabled on cancelled orders. Fixed in the same change: the manual-order route referenced the checkout-only `reusedOrder` variable, a ReferenceError that had silently suppressed the owner-notification email for admin-created orders since the duplicate-guard commit. 1 new test. | 446/446 unit |
