...
| **Payfast live payment verification — reopened** | Previously marked Closed on the belief the live path was "verified end-to-end with a real transaction." The one real transaction on record (Aug 26, order `691b035e`) actually failed ITN signature verification — Payfast confirms it as paid, this server doesn't, so the order never auto-transitions to `paid`. See the correction row in §2 Evolution History. Needs the business owner to check `PAYFAST_PASSPHRASE`/`PAYFAST_SANDBOX_PASSPHRASE` in `.env` against the Payfast merchant dashboard before this can be called verified. |
