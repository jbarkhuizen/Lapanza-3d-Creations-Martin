...
| **`INV-0002`/`INV-0003` not found in this dev environment's database** (owner asked these be marked Pending Payment) | Neither invoice number exists in this checkout's `data/lapanza.db` nor its earliest available backup — the local dev DB's invoice sequence starts fresh from `invoiceNumberSeed`. Most likely these are production-only or belong to the pre-app spreadsheet sequence (`invoiceNumberSeed` exists precisely because that history predates this app, see §2.1). Needs running directly against whichever database actually has them — not guessable from this environment. Should be logged as a real Todo/Backlog item (`createdBy: 'Claude'`) once admin access is available; recorded here in the meantime. |
| **Overnight batch (row 120, §2) not click-tested in a real browser** | Per-item catalog save/remove, the New Order client-detail panel, invoice emails, and the "Updating Client Data" toast were all verified by code review + the full automated test suite (299/299) only — this sandbox had no admin login credentials available to this session (reading `.env` or otherwise probing for them was correctly refused), so no live admin-UI click-through happened. Flagged explicitly for a manual pass before this batch is trusted enough to deploy. |
