...
export function createTodo(data, db = getDb()) {
  ...
  db.prepare(
    `INSERT INTO todo_items (id, number, category, name, description, status, planned_fix_date, actual_fix_date, date_added, created_at, updated_at)
     VALUES (@id, @number, @category, @name, @description, @status, @planned_fix_date, @actual_fix_date, @date_added, @created_at, @updated_at)`,
  ).run({
    id,
...
export function updateTodo(id, data, db = getDb()) {
  ...
  db.prepare(
    `UPDATE todo_items SET category = @category, name = @name, description = @description, status = @status,
