...
export function createResource(data, db = getDb()) {
  ...
  db.prepare(
    `INSERT INTO resources (id, title, description, image_path, file_path, print_settings, filament_type, dimensions, active, sort_order, created_at, updated_at)
     VALUES (@id, @title, @description, @image_path, @file_path, @print_settings, @filament_type, @dimensions, @active, @sort_order, @created_at, @updated_at)`,
  ).run({
    id,
...
export function updateResource(id, data, db = getDb()) {
  ...
  db.prepare(
    `UPDATE resources SET title = @title, description = @description, image_path = @image_path, file_path = @file_path,
